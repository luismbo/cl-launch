...
The fasl for a file \$DIR/\$FILE with be stored in directory
	\$HOME/.cache/common-lisp/\$IMPL/\$DIR/
...
\$HOME/.cache/common-lisp/ by setting and exporting the environment variable
	\$LISP_FASL_CACHE
...
Note that you may use symlinks to move ~/.cache/common-lisp/ to the place of your
...
LISP_FASL_CACHE=/var/cache/common-lisp # assuming precompiled fasls there
'
...
present in the cache (typically in ~/.cache/common-lisp/ecl*). The current
...
be initialized according to LISP_FASL_CACHE or its default of ~/.cache/common-lisp/
...
(defun lisp-version-string ()
  ...
  #+allegro   (format nil
                      ...
                      (if (eq excl:*current-case-mode* :case-sensitive-lower) "M" "A") ; ANSI vs MoDeRn
                      (excl:ics-target-case (:-ics "8") (:+ics ""))
